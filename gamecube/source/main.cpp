...
#define click() PAD_ButtonsDown(0) & PAD_BUTTON_A
#define release() PAD_ButtonsUp(0) & PAD_BUTTON_A
#define hold() PAD_ButtonsHeld(0) & PAD_BUTTON_A
...
int getHighscore()
{
     ...
     int tempScore = 0;
     std::ifstream save;
     save.open("terr-fried.sav", std::ifstream::in );
...
int main(void)
{
    ...
    while (1)   
    {
        ...
        touchX += PAD_StickX(0)/10;
		touchY += -PAD_StickY(0)/10;
        
      
        if (touchX > mouseDownX + 300)
        {
            touchX = mouseDownX + 300;
        }
        if (touchX < mouseDownX - 300)
        {
            touchX = mouseDownX - 300;
        }
        if (touchY > mouseDownY + 300)
        {
            touchY = mouseDownY + 300;
        }
        if (touchY < mouseDownY - 300)
        {
            touchY = mouseDownY - 300;
